        ...
        (0U != ((uint32_t)kLPUART_IdleLineInterruptEnable & enabledInterrupts)))
    {
        ...
        while ((0U != handle->rxDataSize) && (0U != count))
        {
            ...
            else
            {
                ...
                handle->rxData = &handle->rxData[tempCount * 2u];
            }
...
/* Get the bytes that available at this moment. */
        ...
        while ((0U != handle->txDataSize) && (0U != count))
        {
            ...
            {
                LPUART_WriteNonBlocking16bit(base, (const uint16_t *)handle->txData, tempCount);
                handle->txData = &handle->txData[tempCount * 2u];
            }
