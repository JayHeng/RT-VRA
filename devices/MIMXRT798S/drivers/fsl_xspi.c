                        ...
                        bool enableDDR, bool enableX16Mode, uint32_t xspiRootClk)
{
    ...
    if (ptrDllConfig->dllMode == kXSPI_BypassMode)
    {
        ...
        offsetDelayElementCount  = (ptrDllConfig->useRefValue) ?
        ...
        delayElementCoarseAdjust = (ptrDllConfig->useRefValue) ?
        ...
        delayElementFineAdjust   = (ptrDllConfig->useRefValue) ?
    ...
    else
    {
        ...
        refCounterValue = (ptrDllConfig->useRefValue) ?
        ...
        resolutionValue = (ptrDllConfig->useRefValue) ?
...
status_t XSPI_SetDeviceConfig(XSPI_Type *base, xspi_device_config_t *devConfig)
{
    ...
    tmp32 |= XSPI_MCR_DQS_LAT_EN(devConfig->sampleClkConfig.enableDQSLatency) |
             XSPI_MCR_DQS_FA_SEL(devConfig->sampleClkConfig.sampleClkSource & 0x3UL);
    base->MCR = tmp32;
                          ...
                          xspi_sfp_frad_config_t *ptrSfpFradConfig)
{
    ...
    if (enableSFP)
    {
        if (enableMdad)
        {
            ...
            uint32_t tgMdadRegOffset[] = XSPI_TG_MDAD_REG_OFFSET_ARRAY;
            uint32_t tgMdadRegAddr     = 0UL;

            for (i = 0U; i < XSPI_TARGET_GROUP_COUNT; i++)
            {
                tgMdadRegAddr = (uint32_t)base + tgMdadRegOffset[i];
...
void XSPI_GetFradLastTransactionsInfo(XSPI_Type *base, xspi_frad_transaction_info_t *ptrInfo, uint8_t fradId)
{
    ...
    ptrInfo->masterId =
        ((*(uint32_t *)fradWord5RegAddr) & XSPI_FRAD0_WORD5_CMP_MDID_MASK) >> XSPI_FRAD0_WORD5_CMP_MDID_SHIFT;
...
{
    uint32_t tgSfarsRegOffset[] = XSPI_TGSFARS_REG_OFFSET;

    uint32_t tgSfarsRegAddr = (uint32_t)base + (uint32_t)tgSfarsRegOffset[(uint8_t)(tgId)];
...
void XSPI_GetTgAddrWriteStatus(XSPI_Type *base, xspi_target_group_t tgId, xspi_tg_add_write_status_t *ptrStatus)
{
    ...
    ptrStatus->managerId       = tmp32 & XSPI_TGSFARS_TG_MID_MASK;
...
{
    uint32_t tgSfarsRegOffset[]     = XSPI_TGSFARS_REG_OFFSET;
    uint32_t tgIpcrsRegOffset[]     = XSPI_TGIPCRS_REG_OFFSET;
    uint32_t sfpTgIpcrRegOffset[]   = XSPI_SFP_TG_IPCR_REG_OFFSET;
    uint32_t sfpTgIpSfarRegOffset[] = XSPI_SFP_TG_SFAR_REG_OFFSET;
    uint32_t tgMdadRegOffset[]      = XSPI_TG_MDAD_REG_OFFSET_ARRAY;

    uint32_t tgSfarsRegAddr     = (uint32_t)base + (uint32_t)tgSfarsRegOffset[(uint8_t)tgId];
    uint32_t tgIpcrsRegAddr     = (uint32_t)base + (uint32_t)tgIpcrsRegOffset[(uint8_t)tgId];
    uint32_t sfpTgIpcrRegAddr   = (uint32_t)base + (uint32_t)sfpTgIpcrRegOffset[(uint8_t)tgId];
    uint32_t sfpTgIpSfarRegAddr = (uint32_t)base + (uint32_t)sfpTgIpSfarRegOffset[(uint8_t)tgId];
    uint32_t tgMdadRegAddr      = (uint32_t)base + (uint32_t)tgMdadRegOffset[(uint8_t)tgId];
...
status_t XSPI_ReadBlocking(XSPI_Type *base, uint8_t *buffer, size_t size)
{
    ...
    while ((1UL != (base->SR & XSPI_SR_BUSY_MASK)) && ((base->SR & XSPI_SR_IP_ACC_MASK) >> XSPI_SR_IP_ACC_SHIFT))
    {
    }
...
status_t XSPI_TransferBlocking(XSPI_Type *base, xspi_transfer_t *xfer)
{
    ...
    if ((xfer->cmdType == kXSPI_Write) || (xfer->cmdType == kXSPI_Config))
    {
        ...
        status = XSPI_WriteBlocking(base, (uint8_t *)ptrBuffer, xfer->dataSize);
    }
    else if (xfer->cmdType == kXSPI_Read)
    {
        ...
        while (dataSize > 0UL)
        {
            ...
            status = XSPI_ReadBlocking(base, (uint8_t *)ptrBuffer, transferSize);
                                 ...
                                 xspi_ahbBuffer_config_t *ptrBuffer3Config)
{
    ...
    uint16_t subBufferStartAddr; /* Specify the upper 16-bits of the start addresss range. */
    uint16_t subBufferEndAddr;   /* Specify the upper 16-bits of the end addresss range. */
...
void XSPI_GetAhbRequestSuspendInfo(XSPI_Type *base, xspi_ahb_request_suspend_info_t *ptrSuspendInfo)
{
    ...
    {
        ptrSuspendInfo->subBufferId = ((tmp32 & XSPI_SPNDST_SPDSBUF_MASK) >> XSPI_SPNDST_SPDSBUF_SHIFT);
        ptrSuspendInfo->ahbBufferId = ((tmp32 & XSPI_SPNDST_SPDBUF_MASK) >> XSPI_SPNDST_SPDBUF_SHIFT);
        ptrSuspendInfo->dataLeft    = ((tmp32 & XSPI_SPNDST_DATLFT_MASK) >> XSPI_SPNDST_DATLFT_SHIFT);
    ...
    XSPI_Type *base, uint16_t buf0Size, uint16_t buf1Size, uint16_t buf2Size, uint16_t buf3Size)
{
    ...
    base->BUFIND[1] = XSPI_BUFIND_TPINDX(buf0Size + buf1Size);
    base->BUFIND[2] = XSPI_BUFIND_TPINDX(buf0Size + buf1Size + buf2Size);
...
void XSPI_EnableAhbBufferPerfMonitor(XSPI_Type *base, uint8_t ahbBufferId, uint8_t subBufferId)
{
    ...
    tmp32 |= XSPI_AHB_PERF_CTRL_BUF0_EN_MASK << (uint32_t)(ahbBufferId);
