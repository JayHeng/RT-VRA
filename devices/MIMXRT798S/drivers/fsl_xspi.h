...
/*! @name Driver version */
#define FSL_XSPI_DRIVER_VERSION (MAKE_VERSION(2, 2, 0))
/*@{*/
...
typedef struct _xspi_ahbBuffer_config
{
    ...
    uint16_t bufferSize;                                     /* Specify the AHB buffer size and transfer size,
...
{
    base->AHB_PERF_CTRL &= ~(XSPI_AHB_PERF_CTRL_BUF0_EN_MASK << (uint32_t)(ahbBufferId));
}
...
{
    return (bool)((base->AHB_PERF_CTRL & (XSPI_AHB_PERF_CTRL_BUF0_MISS_OVF_MASK << (uint32_t)ahbBufferId)) != 0UL);
}
